...
### cracking modes
`Single Crack Mode` is one of the most common John modes used when attempting to crack passwords using a single password list. It is a brute-force attack

```shell-session
john --format=<hash_type> <hash or hash_file>
```
- wordlists specified with the `--wordlist` option
- It will use any rules set with the `--rules` option (if any rules are given) to generate further candidate passwords.
...
It is also possible to crack even password-protected or encrypted files with John. it will automatically detect the format of the file and tries to crack
...
```
`find the tool installed or not hear `
```shell-session
locate *2john*
````
